...
class CanvasImage():
    ...
    def Next(self, index, ListImg):
        if index != 0:
            self.destroy()
        if (index == len(ListImg)):
            index = 0
        self.Run(ListImg, index)


    def Run(self, ListImg, count):
        """ Initialize the ImageFrame """
        #a=open("x1.txt","r")
        #data=a.readlines()
        path = str(ListImg[count])
        self.imscale=1.0
        ...
        self.__imframe = tk.Frame()  # placeholder of the ImageFrame object
        #self.__imframe = self.master
        ...
        f=tk.Button(self.__imframe,text="Next",command=lambda val = ListImg,count = count + 1:self.Next(count, val),height=3,width=10)
        f.grid(row=4, column=0)
        
        def clickBack():
            self.destroy()
        Backbutton = tk.Button(self.__imframe, text="Back", font=("Arial Bold", 12), command=lambda:clickBack())
        Backbutton.grid(row=4, column=1, pady=5)
...
class MainWindow(tk.Frame):
    """ Main window class """
    def __init__(self, mainframe, ListImg):
        ...
        def clicked(index):
            canvas = CanvasImage(self.master, ListImg[index])
            canvas.grid(row=0, column=0)
            if (index == len(ListImg) - 1):
                index = -1
            self.master.button = tk.Button(self.master, text='NEXT', command=lambda index = index + 1: clicked(index))
            self.master.button.grid(row=1, column=0)
            self.master.button.config(height=2, width=20)

        self.master.button = tk.Button(self.master, text='NEXT', command=lambda count = count + 1: clicked(count))
        self.master.button.grid(row=1, column=0)
        self.master.button.config(height=2, width=20)


def runImage(ListImg):
    app = CanvasImage()
    app.Next(index=0,ListImg=ListImg)
    #app.mainloop()

# if __name__ == '__main__':
#     ListImg = ["images/VN-13_1.jpg", "images/VN-13_2.jpg", "images/VN-13_3.jpg", "images/VN-13_4.jpg"]
#     runImage(ListImg)


#app = MainWindow(tk.Tk(), path="vinh-ha-long-o-tinh-nao-viet-nam.jpeg")
#app.mainloop()
