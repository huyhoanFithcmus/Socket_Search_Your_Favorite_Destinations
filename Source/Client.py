...
class GUI(tk.Frame):
    ...
    def populate(self, type):
        ...
        def clicked(listImg):
            for i in range(0, len(listImg)):
                ...
                listImg[i] = re.sub(r"^.+?/", "/", listImg[i])
                listImg[i] = listImg[i].replace('/','')
                print(listImg[i]) 
    ...
    def mainMenu (self):
        ...
        Infobutton = tk.Button(self.frame, text="Thông tin nhóm", font=("Arial Bold", 12), command=lambda: self.creator())
